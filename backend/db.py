    ...
    async with pool.acquire() as conn:
        ...
        for _, row in data.iterrows():
            await conn.execute(
                ...
                battery_capacity,year,gpu_type,gpu,rating,price)
                ...
                row['year'],row['gpu_type']=="Dedicated",row['gpu'],row['rating'],row['price'])
    ...
    async with pool.acquire() as conn:
        await conn.execute(
            """UPDATE laptop SET title=$2,model_number=$3,model_name=$4,
                ...
                battery_capacity=$17,year=$18,gpu_type=$19,gpu=$20,rating=$21,
                price=$22 WHERE asin = $1
            """,
            ...
            laptop.battery_capacity,laptop.year,laptop.gpu_type,laptop.gpu,laptop.rating,
            laptop.price
            )
...
async def load_all():
    global pool
    async with pool.acquire() as conn:
        rows = await conn.fetch("SELECT * FROM laptop")
